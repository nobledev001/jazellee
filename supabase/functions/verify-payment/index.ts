// Supabase Edge Function: verify-payment (Alias / mirror of verify-paystack)
...
async function checkPersistentRateLimit(
  ...
  try {
    ...
    console.warn('[verify-payment] Database rate limit check fallback to memory:', err);
  }
...
export async function handler(req: Request): Promise<Response> {
  ...
  if (req.method === 'GET') {
    return new Response(
      JSON.stringify({
        status: 'ok',
        service: 'verify-payment edge function',
  ...
  try {
    ...
    if (!ipLimit.allowed || !refLimit.allowed) {
      ...
      console.warn(`[verify-payment] Rate limit exceeded (${ipLimit.source}) for IP: ${clientIp}, Ref: ${cleanRef}`);
      return new Response(
        JSON.stringify({
          verified: false,
          status: 'rate_limited',
    ...
    if (!secretKey) {
      console.error('[verify-payment] PAYSTACK_SECRET_KEY is not configured on server');
      return new Response(
        JSON.stringify({
          verified: false,
    ...
    console.log(`[verify-payment] Calling Paystack verify API for reference: ${cleanRef}`);
