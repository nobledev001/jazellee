...
export default function TrackOrderPage() {
  ...
  const findOrder = async (id: string) => {
    setLoading(true);
    const { data } = await supabase.from('orders').select('*').eq('order_number', id.trim()).maybeSingle();
  ...
  useEffect(() => {
    if (initialId) findOrder(initialId);
  }, [initialId]);
      ...
      <div className="mx-auto max-w-2xl">
        ...
        <div className="mt-6 flex gap-3">
          <div className="relative flex-1">
            ...
            <input
              ...
              onKeyDown={(e) => e.key === 'Enter' && findOrder(inputId)}
              placeholder="e.g. JAZ-12345678"
              className="input-jazelle pl-12"
            />
          </div>
          <button onClick={() => findOrder(inputId)} className="btn-primary" disabled={loading}>
        ...
        {searched && !order && (
          <div className="mt-6 rounded-4xl bg-white p-8 text-center shadow-soft">
            <p className="text-berry-500">No order found with that ID.</p>
            <p className="mt-1 text-xs text-berry-400">Double-check your order ID from your confirmation email or page.</p>
          </div>
        )}
            ...
            <div className="mt-6 rounded-4xl bg-white p-6 shadow-soft">
              <div className="flex items-center justify-between">
                ...
                <div className="text-right">
                  ...
                  <p className="text-sm font-medium text-berry-700">{new Date(order.created_at).toLocaleDateString('en-NG', { day: 'numeric', month: 'long', year: 'numeric' })}</p>
                </div>
              </div>
            </div>
                  ...
                  return (
                    <div key={step.key} className="flex gap-4">
                      <div className="flex flex-col items-center">
                        <span className={`flex h-10 w-10 items-center justify-center rounded-full transition-all ${isComplete ? 'bg-blush-500 text-white' : 'bg-blush-50 text-blush-300'} ${isCurrent ? 'ring-4 ring-blush-100' : ''}`}>
                          <Icon className="h-5 w-5" />
                        </span>
                        {!isLast && <span className={`my-1 w-0.5 flex-1 ${index < currentStepIndex ? 'bg-blush-400' : 'bg-blush-100'}`} style={{ minHeight: '2.5rem' }} />}
                      </div>
                      <div className={`pb-6 ${isLast ? 'pb-0' : ''}`}>
                        <p className={`text-sm font-semibold ${isComplete ? 'text-berry-700' : 'text-berry-300'}`}>{step.label}</p>
                        <p className={`text-xs ${isComplete ? 'text-berry-400' : 'text-blush-200'}`}>{step.description}</p>
                        {isCurrent && (
